...
def is_digit(user_input_number: str):
    ...
    for char in str(user_input_number):
...
def is_duplicated_number(three_digit: str):
    ...
    compare = (three_digit[0] == three_digit[1])\
        or (three_digit[0] == three_digit[2])\
        or (three_digit[1] == three_digit[2])
...
def get_not_duplicated_three_digit_number():
    ...
    while True:
        generated = get_random_number()
        if is_validated_number(generated):
            result = int(generated)
            break
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    for i in range(3):
        if random_number.find(user_input_number[i]) >= 0\
            and user_input_number[i] != random_number[i]:
            ball += 1
...
def main():
    ...
    while not finish:
        ...
        if correct:
            while not finish:
                retry_input = input("You win, one more(Y/N)?")

                if is_yes(retry_input):
                    break
                elif is_no(retry_input):
                    finish = True
                    continue
                elif retry_input == '0':
                    finish = True
